...
test('Studio offers a restrained set of distinct optional palettes', () => {
  assert.deepEqual(Object.keys(themes), ['neutral', 'nexus', 'aurora', 'ember', 'verdant']);
  assert.equal(new Set(Object.values(themes).map((theme) => theme.tokens['--nx-accent'])).size, 5);
});
...
test('Studio action groups keep related controls visually separated', () => {
  const css = readFileSync(new URL('../../styles.css', import.meta.url), 'utf8');
  assert.match(css, /\.studio-action-stack\s*\{[\s\S]*?gap:\s*1\.25rem/);
  assert.match(css, /\.studio-action-section\s*\{[\s\S]*?gap:\s*\.75rem/);
});
